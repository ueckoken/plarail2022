...
class Signal:
    def __init__(
        self, sourceSectionId: Section.SectionId, targetSectionId: Section.SectionId, value: Literal["R", "G"]
    ):
...
class SignalSystem:
    ...
    def getSignal(self, sourceSectionId: Section.SectionId, targetSectionId: Section.SectionId) -> Signal:
        ...
        if (
            junction.outSectionStraight == None
            or junction.outSectionStraight.id != targetSectionId
        ):
            if (
                junction.outSectionCurve == None
                or junction.outSectionCurve.id != targetSectionId
            ):
                return None
        # 信号判定
